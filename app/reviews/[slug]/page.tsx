...
export async function generateMetadata({
    ...
    const casino = TOP6_CASINOS_MOCK.find((c) => c.slug === slug);
    ...
    const author =
        authors.find((a) => a.name === casino.review.author.name) || authors[0];

    return {
        title: `${
            casino.brand
        } Review ${new Date().getFullYear()} - Expert Analysis & Rating`,
        description: `Comprehensive ${casino.brand} casino review by ${
            author.name
        }. ${casino.review.summary} Rating: ${
            casino.review.overallRating
        }/10. Updated ${new Date(
            casino.review.lastUpdated
        ).toLocaleDateString()}.`,
        keywords: [
            `${casino.brand} review`,
            `${casino.brand} casino`,
        ...
        openGraph: {
            title: `${
                casino.brand
            } Casino Review ${new Date().getFullYear()} - ${
                casino.review.overallRating
            }/10 Rating`,
