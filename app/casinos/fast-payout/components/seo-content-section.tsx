...
import { ChevronDown, ChevronUp, Clock, Shield, Star, CheckCircle, ExternalLink } from "lucide-react"
...
export function SEOContentSection({ content, variant = "default", language = "en" }: SEOContentSectionProps) {
  return (
...
function TableOfContents({ items }: { items: { id: string; title: string; level: number }[] }) {
      ...
      <CardHeader className="pb-3">
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" className="w-full justify-between p-0 h-auto font-semibold text-foreground">
              Table of Contents
              ...
              {items.map((item) => (
                <a
                  ...
                  className={`block text-sm text-muted-foreground hover:text-foreground transition-colors ${
                    item.level === 2 ? "font-medium" : item.level === 3 ? "ml-4" : "ml-8"
                  }`}
                >
                  {item.title}
                </a>
...
function ContentSectionRenderer({ section }: { section: ContentSection }) {
  const HeadingComponent = section.heading.level === 2 ? "h2" : section.heading.level === 3 ? "h3" : "h4"
  ...
  return (
    <section id={section.heading.id} className="scroll-mt-20">
      <HeadingComponent className={headingClasses[section.heading.level]}>{section.heading.text}</HeadingComponent>
      ...
      {section.type === "guide" && section.content.guide && <GuideSection guide={section.content.guide} />}

      {section.type === "testimonials" && section.content.testimonials && (
        <TestimonialsSection testimonials={section.content.testimonials} />
      )}

      {section.type === "comparison" && section.content.comparison && (
        <ComparisonTableSection comparison={section.content.comparison} />
      )}

      {section.type === "trust-signals" && section.content.trustSignals && (
        <TrustSignalsSection signals={section.content.trustSignals} />
      )}

      {section.type === "faq" && section.content.faq && <FAQSection faqs={section.content.faq} />}
          ...
          <CardContent>
            ...
            {step.warning && (
              <div className="mt-3 p-3 bg-destructive/10 border border-destructive/20 rounded-md">
                <p className="text-sm text-destructive font-medium">⚠️ {step.warning}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <table className="w-full border-collapse border border-border rounded-lg">
          <thead>
            <tr className="bg-muted/50">
              {comparison.headers.map((header: string, index: number) => (
                <th key={index} className="border border-border p-3 text-left font-medium text-foreground">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {comparison.rows.map((row: any, index: number) => (
              <tr key={index} className={row.highlighted ? "bg-primary/5" : "bg-card"}>
                <td className="border border-border p-3 font-medium text-foreground">{row.casino}</td>
                {row.data.map((cell: any, cellIndex: number) => (
                  <td key={cellIndex} className="border border-border p-3 text-muted-foreground">
                    {cell}
                  </td>
                ))}
              </tr>
...
function TrustSignalsSection({ signals }: { signals: any[] }) {
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mt-4">
      {signals.map((signal) => (
        <Card key={signal.id} className="bg-card border-border text-center">
          <CardContent className="p-4">
            <div className="w-12 h-12 mx-auto mb-3 bg-primary/10 rounded-lg flex items-center justify-center">
              <Shield className="w-6 h-6 text-primary" />
            </div>
            <h4 className="font-medium text-foreground mb-2">{signal.title}</h4>
            ...
            {signal.verificationUrl && (
              <Button variant="outline" size="sm" asChild className="text-xs bg-transparent">
                <a href={signal.verificationUrl} target="_blank" rel="noopener noreferrer">
            ...
            <CollapsibleTrigger asChild>
              <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors">
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-foreground text-left">{faq.question}</h4>
...
function PaymentMethodsSection({ methods }: { methods: PaymentMethodInfo[] }) {
  return (
    <section className="scroll-mt-20">
      <h2 className="text-2xl font-bold text-foreground mb-4">Payment Methods Comparison</h2>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-foreground">{method.averageTime}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Min/Max:</span>
                  <span className="text-sm font-medium text-foreground">
                    {method.minWithdrawal} - {method.maxWithdrawal}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Fees:</span>
                  <span className="text-sm font-medium text-foreground">{method.fees}</span>
                </div>

                <div className="pt-3 border-t border-border">
                  <h5 className="text-sm font-medium text-foreground mb-2">Pros:</h5>
