...
} from '../../../lib/ai-generation/master-prompt';
    ...
    try {
        ...
        if (!pageContent || !casinos || !criteria) {
            return NextResponse.json(
                {
                    success: false,
                    error: 'Missing required fields: pageContent, casinos, or criteria',
                },
                { status: 400 }
            );
        }
        ...
        if (!process.env.OPENAI_API_KEY) {
            console.error(
                '[v0] OPENAI_API_KEY environment variable is not set'
            );
            return NextResponse.json(
        ...
        const author =
            getAuthorById(pageContent.author.id) || pageContent.author;
        const language =
            getLanguageByCode(pageContent.language.code) ||
            pageContent.language;
        const geo =
            getGeoByCountryCode(pageContent.geo.countryCode) || pageContent.geo;
        ...
        console.log(
            '[v0] Making AI request with structured output using generateObject...'
        );

        const result = await generateObject({
            model: openai('gpt-5'),
            system: `You are an expert iGaming analyst. Generate comprehensive casino criteria content based on the provided context and your research. 

CRITICAL SCHEMA REQUIREMENTS - MUST FOLLOW EXACTLY:
1. ALL keyStats arrays MUST contain exactly 2-4 elements (never just 1)
   - Example: [{"label": "Trust Score", "value": "96", "unit": "/100"}, {"label": "License", "value": "MGA"}]
2. methodologyUpdatedAt MUST be full ISO datetime format (e.g., "2023-09-15T10:00:00Z")
3. eeat.trustSignals object is REQUIRED:
   - Example: {"auditBadges": ["eCOGRA", "GLI"], "adrProvider": "IBAS", "complaintsWindowDays": 90}
...
Follow the schema exactly and ensure all required fields are populated with accurate, engaging content. If you are missing some information you can reference the provided context or make your own educated assumptions to keep the content coherent and valid output schema.`,
            ...
            if (error?.message?.includes('response did not match schema')) {
                ...
                console.error(
                    '[v0] Consider simplifying the schema or making fields optional'
                );

                // Log the specific validation errors for debugging
                if (error?.cause?.issues) {
                    console.error('[v0] Detailed validation errors:');
                    error.cause.issues.forEach((issue: any, index: number) => {
                        console.error(
                            `  ${index + 1}. Path: ${issue.path.join('.')} - ${
                                issue.message
                            }`
                        );
                    });
        ...
        if (generatedData) {
            ...
            if (generatedData.items?.[0]) {
                console.log('[v0] First item structure:', {
                    ...
                    hasWinnerCasinoId: !!generatedData.items[0].winnerCasinoId,
                    hasProofText: !!generatedData.items[0].proofText,
        ...
        try {
            ...
            console.error(
                '[v0] Generated content validation failed:',
                validationError
            );
            return NextResponse.json(
                {
